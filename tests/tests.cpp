...
{
    REQUIRE("东南" == SolarDay::from_ymd(2021, 11, 13)
                          .get_lunar_day()
                          .get_sixty_cycle()
                          .get_heaven_stem()
...
TEST_CASE("eight_char_test_0")
{
    ...
    // 年柱地支十神（中气)
    REQUIRE("正财" ==
            me.get_ten_star(year.get_earth_branch().get_hide_heaven_stem_middle().value()).get_name());
    // 年柱地支十神（余气)
    REQUIRE("正官" ==
            me.get_ten_star(year.get_earth_branch().get_hide_heaven_stem_residual().value()).get_name());
...
TEST_CASE("phenology_test_0")
{
    ...
    REQUIRE("2020年4月19日 22:45:21" ==
            phenology.get_phenology().get_julian_day().get_solar_time().to_string());
    // 该候的第5天
...
{
    REQUIRE("乙巳年戊寅月癸卯日" ==
            SixtyCycleDay::from_solar_day(SolarDay::from_ymd(2025, 2, 3)).to_string());
}
